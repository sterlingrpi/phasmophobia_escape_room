#https://www.tensorflow.org/hub/tutorials/object_detection
import tensorflow as tf
import tensorflow_hub as hub
import time
import matplotlib.pyplot as plt
import numpy as np
from PIL import Image, ImageDraw, ImageColor, ImageFont
from cv2 import VideoCapture

class Camera:
    def __init__(self,
                 camera_port = 0):
        self.camera = VideoCapture(camera_port)
        self.image = None
        module_handle = "https://tfhub.dev/google/openimages_v4/ssd/mobilenet_v2/1"
        self.detector = hub.load(module_handle).signatures['default']

    def capture_image(self):
        _, self.image = self.camera.read()

    def get_image(self):
        return self.image

    def display_image(self):
        fig = plt.figure(figsize=(20, 15))
        plt.grid(False)
        plt.imshow(self.image)
        plt.show()

    def draw_bounding_box_on_image(self,
                                   image_pil,
                                   ymin,
                                   xmin,
                                   ymax,
                                   xmax,
                                   color,
                                   font,
                                   thickness=4,
                                   display_str_list=()):
        """Adds a bounding box to an image."""
        draw = ImageDraw.Draw(image_pil)
        im_width, im_height = image_pil.size
        (left, right, top, bottom) = (xmin * im_width, xmax * im_width,
                                      ymin * im_height, ymax * im_height)
        draw.line([(left, top), (left, bottom), (right, bottom), (right, top), (left, top)],
                  width=thickness,
                  fill=color)

        # If the total height of the display strings added to the top of the bounding
        # box exceeds the top of the image, stack the strings below the bounding box
        # instead of above.
        display_str_heights = [font.getsize(ds)[1] for ds in display_str_list]
        # Each display_str has a top and bottom margin of 0.05x.
        total_display_str_height = (1 + 2 * 0.05) * sum(display_str_heights)

        if top > total_display_str_height:
            text_bottom = top
        else:
            text_bottom = top + total_display_str_height
        # Reverse list and print from bottom to top.
        for display_str in display_str_list[::-1]:
            text_width, text_height = font.getsize(display_str)
            margin = np.ceil(0.05 * text_height)
            draw.rectangle([(left, text_bottom - text_height - 2 * margin), (left + text_width, text_bottom)],
                           fill=color)
            draw.text((left + margin, text_bottom - text_height - margin),
                      display_str,
                      fill="black",
                      font=font)
            text_bottom -= text_height - 2 * margin

    def draw_boxes(self, boxes, class_names, scores, max_boxes=10, min_score=0.1):
        """Overlay labeled boxes on an image with formatted scores and label names."""
        colors = list(ImageColor.colormap.values())

        # font = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSansNarrow-Regular.ttf", 25)
        font = ImageFont.load_default()

        for i in range(min(boxes.shape[0], max_boxes)):
            if scores[i] >= min_score:
                ymin, xmin, ymax, xmax = tuple(boxes[i])
                display_str = "{}: {}%".format(class_names[i].decode("ascii"),
                                         int(100 * scores[i]))
                color = colors[hash(class_names[i]) % len(colors)]
                image_pil = Image.fromarray(np.uint8(self.image)).convert("RGB")
                self.draw_bounding_box_on_image(image_pil,
                                                ymin,
                                                xmin,
                                                ymax,
                                                xmax,
                                                color,
                                                font,
                                                display_str_list=[display_str])
                np.copyto(self.image, np.array(image_pil))

    def run_detector(self):
            converted_img  = tf.image.convert_image_dtype(self.image, tf.float32)[tf.newaxis, ...]
            start_time = time.time()
            result = self.detector(converted_img)
            end_time = time.time()
            # print("Inference time: ", end_time - start_time)

            result = {key:value.numpy() for key,value in result.items()}

            indices = [i for i, value in enumerate(result['detection_class_entities']) if value == b'Human face']

            result_filtered = {}
            for key in result:
                result_filtered[key] = np.array([value for i, value in enumerate(result[key]) if i in indices])
            # print(result_filtered)
            #
            # print("Found %d objects." % len(result_filtered["detection_scores"]))
            self.draw_boxes(result_filtered["detection_boxes"],
                            result_filtered["detection_class_entities"],
                            result_filtered["detection_scores"])

if __name__ == '__main__':
    camera = Camera()
    camera.capture_image()
    camera.display_image()
    while True:
        camera.capture_image()
        camera.run_detector()
        camera.display_image()